...
if ($q !== '') {
    $where .= " AND (a.title LIKE ? OR a.description LIKE ? OR c.group_name LIKE ? OR CAST(a.id AS CHAR) LIKE ?) ";
    $like = "%{$q}%";
...
if ($year > 0) {
    $where .= " AND YEAR(a.achieved_on) = ? ";
    $params[] = $year;
    $types   .= 'i';
}
if ($month >= 1 && $month <= 12) {
    $where .= " AND MONTH(a.achieved_on) = ? ";
    $params[] = $month;
    $types   .= 'i';
}
...
$clubs = [];
$cq = $conn->query("
    SELECT c.id, c.group_name, COUNT(a.id) AS cnt
    FROM clubs c
    LEFT JOIN achievements a ON a.club_id = c.id
...
// Counts
$gt = (int)($conn->query("SELECT COUNT(*) AS c FROM achievements")->fetch_assoc()['c'] ?? 0);
$byApproval = ['pending'=>0,'approved'=>0,'rejected'=>0];
$appr = $conn->query("SELECT approval_status, COUNT(*) c FROM achievements GROUP BY approval_status");
while ($r = $appr->fetch_assoc()) $byApproval[$r['approval_status']] = (int)$r['c'];

// Pagination
$stmtC = $conn->prepare("SELECT COUNT(*) AS c FROM achievements a JOIN clubs c ON c.id=a.club_id {$where}");
...
// Fetch page
$sql = "SELECT a.id, a.club_id, a.title, a.description, a.achieved_on, a.created_at,
               a.approval_status, a.rejection_reason,
               c.group_name
        FROM achievements a
...
function statusFilterLabel($s){ return $s === 'all' ? 'All Achievements' : ucfirst($s) . ' Achievements'; }
?>
<main class="main-content container-fluid">
    <style>
        ...
        @media(max-width:768px){.table-stack thead{display:none}.table-stack,.table-stack tbody,.table-stack tr,.table-stack td{display:block;width:100%}.table-stack tr{background:#fff;margin-bottom:.9rem;border-radius:10px;border:1px solid #e9ecef;box-shadow:0 2px 8px rgba(0,0,0,.03);padding:.25rem .5rem}.table-stack td{display:flex;justify-content:space-between;align-items:center;padding:.5rem .25rem;border:none!important;border-bottom:1px dashed #eef2f6!important}.table-stack td:last-child{border-bottom:none!important}.table-stack td::before{content:attr(data-label);font-weight:600;margin-right:1rem;color:#0f2f47;min-width:80px;flex-shrink:0}}
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to <strong class="text-success">approve</strong> this achievement?</p>
                    <p class="text-muted small mb-0">This achievement will be visible to all users.</p>
                </div>
    ...
    <div class="modal fade confirmation-modal reject" id="rejectModal" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to <strong class="text-danger">reject</strong> this achievement?</p>
                    <div class="mb-3">
    ...
    <div class="row g-3 mb-4">
        <div class="col-12 col-md-3"><div class="card stat-card total"><div class="card-body"><div>Total Achievements</div><div class="h3 mb-0"><?= number_format($gt) ?></div></div></div></div>
    ...
    <div class="card shadow-sm border-0 mb-3">
        <form class="card-body row g-2">
            ...
            <div class="col-lg-2 d-flex align-items-end gap-2">
                ...
                <a href="manage_achievements.php" class="btn btn-light flex-fill"><i class="fa-solid fa-rotate-left me-1"></i> Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white d-flex justify-content-between flex-wrap gap-2">
            <div>
                <strong>Achievement Results</strong> <span class="text-muted">(<?= number_format($total) ?> found)</span>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle table-stack mb-0">
                <thead class="table-light">
                    <tr>
                        <th>ID</th><th>Title</th><th>Club</th><th>Achieved On</th>
                        <th>Approval</th><th>Created</th><th class="text-end">Actions</th>
                    </tr>
                </thead>
                <tbody>
                <?php if (!$rows): ?>
                    <tr><td colspan="7" class="text-center text-muted py-5">No achievements found.</td></tr>
                <?php else: foreach ($rows as $r): ?>
                    <tr>
                        ...
                        <td data-label="Achieved On"><?= h($r['achieved_on'] ? date('M j, Y', strtotime($r['achieved_on'])) : '—') ?></td>
                        ...
                        <td data-label="Actions" class="text-end">
                            <div class="btn-group">
                                <!-- View Button -->
                                <a href="view_achievement.php?id=<?= (int)$r['id'] ?>" 
                                ...
                                <!-- Edit Button -->
                                <a href="edit_achievement.php?id=<?= (int)$r['id'] ?>" 
                                   class="btn btn-sm btn-outline-primary" 
                                   title="Edit">
                                ...
                                <?php if ($r['approval_status']==='pending'): ?>
                                    <button class="btn btn-sm btn-outline-success appr-btn" data-id="<?= (int)$r['id'] ?>" data-title="<?= h($r['title']) ?>" title="Approve">
                                        <i class="fa-solid fa-check"></i>
                                    </button>
                                    <button class="btn btn-sm btn-outline-warning rej-btn" data-id="<?= (int)$r['id'] ?>" data-title="<?= h($r['title']) ?>" title="Reject">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.appr-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            currentAchievementId = btn.dataset.id;
            currentAchievementTitle = btn.dataset.title;
            
            // Update modal content with achievement title
            const modalBody = document.querySelector('#approveModal .modal-body p:first-child');
            if (currentAchievementTitle) {
                modalBody.innerHTML = `Are you sure you want to <strong class="text-success">approve</strong> the achievement: <br><strong>"${currentAchievementTitle}"</strong>?`;
            }
    ...
    document.querySelectorAll('.rej-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (currentAchievementTitle) {
                modalBody.innerHTML = `Are you sure you want to <strong class="text-danger">reject</strong> the achievement: <br><strong>"${currentAchievementTitle}"</strong>?`;
            }
    ...
    document.getElementById('confirmApproveBtn').addEventListener('click', () => {
        if (currentAchievementId) {
            callAPI(currentAchievementId, 'approved', '');
            approveModal.hide();
        }
    });
    ...
    document.getElementById('confirmRejectBtn').addEventListener('click', () => {
        if (currentAchievementId) {
            ...
            callAPI(currentAchievementId, 'rejected', reason);
            rejectModal.hide();
        }
    });
    ...
    async function callAPI(id, status, reason='') {
        ...
        try {
            const resp = await fetch('update_achievement_approval.php', { 
                method:'POST', 
                body: fd, 
